#!/bin/bash
#SBATCH -J 7_sbatch_coverage_analysis		# jobname
...
gene_bed='gwen_genes_thille.bed'

module load bedtools/2.30.0

chromosome=$(awk -F "\t" -v id=${SLURM_ARRAY_TASK_ID} 'FNR == id {print $1}' contigs.txt)
median=$(awk -F "\t" -v id=${SLURM_ARRAY_TASK_ID} 'FNR == id {print $2}' contigs.txt)

awk -v cm=${chromosome} '{ if($1 == cm) print $3 }' gwen_thille_cov_2,3_delete.txt > temp_coverage_${chromosome}.txt
