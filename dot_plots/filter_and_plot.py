...
def filter_and_plot(ref_file, qry_file, contig_file, paf_file):
    # Step 1: Load positions and alignment scores from PAF file format
    paf_data = pd.read_csv(paf_file, sep='\t', header=None, usecols=[0, 2, 3, 5, 7, 8, 9, 10, 11], 
                            names=['query_id', 'query_start', 'query_end', 'reference_id', 'ref_start', 'ref_end', 'num_matches', 'block_length', 'score'])

    # Load contig lengths
    contig_sizes_df = pd.read_csv(contig_file, sep='\t', header=None, names=['Chromosome', 'length'])
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python script.py <ref_file> <qry_file> <contig_file> <paf_file>")
        sys.exit(1)
    ...
    filter_and_plot(ref_file, qry_file, contig_file, paf_file)
