#!/bin/bash
#SBATCH -J blastx 		# jobname
#SBATCH -o blastx.o%A.%a	# jobname.o%j for single (non array) jobs jobname.o%A.%a for array jobs
#SBATCH -e blastx.e%A.%a	# error file name A is the jobid and a is the arraytaskid
#SBATCH -a 1		# start and stop of the array start-end
#SBATCH -A mcb180013p
###SBATCH --nodes=1
###SBATCH --ntasks-per-node=1
#SBATCH -p RM		# queue (partition) -- compute, shared, large-shared, debug (30mins)
###SBATCH --mem-per-cpu=4000 (2500??)
...
OUTFILE=$(basename ${QRY} .fasta)_swissprot_blastx.outfmt6
